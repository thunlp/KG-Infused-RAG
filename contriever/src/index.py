...
print(f"FAISS using GPU: {faiss.get_num_gpus() > 0}")
...
class Indexer(object):
    ...
    def search_knn(self, query_vectors: np.array, top_docs: int, index_batch_size: int = 512) -> List[Tuple[List[object], List[float]]]:
        query_vectors = query_vectors.astype('float32')
        result = []
        nbatch = (len(query_vectors)-1) // index_batch_size + 1
        print(f"n_batch: {nbatch}")
